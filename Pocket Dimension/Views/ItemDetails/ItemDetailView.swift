...
struct ItemDetailView: View {
    ...
    private func locationRow(for location: StorageLocation) -> some View {
        HStack {
            LocationIcon(type: LocationType(rawValue: location.type.rawValue) ?? .room)
            ...
            Button(action: {
                ...
                // For now, just show the location name
            }) {
    ...
    private func locationDetails(for location: StorageLocation) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            // Show parent location if exists
            if let parentId = location.parentId,
               let parentLocation = locationManager.getLocation(by: parentId) {
                Text("in \(parentLocation.name)")
                    .font(.caption)
                    .foregroundColor(.secondary)
                    .italic()
            }
    ...
    private var noLocationRow: some View {
        HStack {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("Tap edit to assign a location")
                    .font(.caption)
                    .foregroundColor(.secondary)
                    .italic()
            }
                ...
                ImagePicker { image in
                    if let imageData = image.jpegData(compressionQuality: 0.7) {
                        ...
                        do {
                            ...
                            setThumbnail() // Refresh thumbnail display
                        } catch {
