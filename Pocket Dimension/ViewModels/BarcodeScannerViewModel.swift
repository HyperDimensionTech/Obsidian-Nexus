...
class BarcodeScannerViewModel: NSObject, ObservableObject {
    ...
    init(scannerService: BarcodeScannerService = BarcodeScannerService()) {
        ...
        super.init()
        print("🟢 BarcodeScannerViewModel: Initializing")
        setupBindings()
    }
    
    deinit {
        print("🔴 BarcodeScannerViewModel: Deallocating")
        // Cancellables will be automatically cancelled when they're deallocated
    }
