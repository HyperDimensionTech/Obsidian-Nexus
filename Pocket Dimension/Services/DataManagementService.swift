...
class DataManagementService: NSObject {
    ...
    func exportData() async throws -> URL {
        // Use CRDT data only - no legacy table access
        let items = try storage.loadItems()
        let locations = try storage.loadLocations()
        
        // Load ISBN mappings from CRDT storage
        let isbnMappings = try storage.loadISBNMappings()
        ...
        // Add ISBN mappings section to the CSV
        if !isbnMappings.isEmpty {
            var mappingsContent = "\n\n--- ISBN Mappings ---\n"
            mappingsContent += "IncorrectISBN,GoogleBooksID,Title,IsReprint,DateAdded\n"
            
            for mapping in isbnMappings {
                let dateAdded = dateFormatter.string(from: mapping.dateAdded)
                let isReprint = mapping.isReprint ? "true" : "false"
                ...
                let formattedMappingComponents = mappingComponents.map { component in
                    "\"\(component.replacingOccurrences(of: "\"", with: "\"\""))\""
                }
                
                mappingsContent += formattedMappingComponents.joined(separator: ",") + "\n"
            }
            
            // Append mappings to the same file
            let fileHandle = try FileHandle(forWritingTo: fileURL)
            defer { fileHandle.closeFile() }
            
            fileHandle.seekToEndOfFile()
            if let mappingsData = mappingsContent.data(using: .utf8) {
                fileHandle.write(mappingsData)
            }
        }
    ...
    func importData(from fileURL: URL) async throws {
        ...
        let allRows = content.components(separatedBy: .newlines)
        ...
        // Find the split point for ISBN mappings
        var isbnMappingsStartIndex: Int?
        for (index, row) in allRows.enumerated() {
            if row.contains("--- ISBN Mappings ---") {
                isbnMappingsStartIndex = index + 2 // Skip the separator and header
                break
            }
        }
        
        // Import inventory items (up to ISBN mappings section)
        let itemRows = isbnMappingsStartIndex.map { Array(allRows[..<$0]) } ?? allRows
        ...
        for row in itemRows.dropFirst() {
            ...
            // Use CRDT only - create events directly
            try storage.save(item)
        }
        
        // Import ISBN mappings if present
        if let startIndex = isbnMappingsStartIndex {
            let mappingRows = Array(allRows[startIndex...])
            ...
            for row in mappingRows {
                ...
                let correctGoogleBooksID = columns[1]
                let title = columns[2]
                let isReprint = columns[3].lowercased() == "true"
                // Note: dateAdded is parsed but CRDT uses current Date() for event timestamp
                
                // Create ISBN mapping using CRDT
