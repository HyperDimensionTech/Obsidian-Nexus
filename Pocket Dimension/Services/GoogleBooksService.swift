...
class GoogleBooksService: ObservableObject {
    ...
    private let apiConfiguration = APIConfiguration.shared
    ...
    init() {
        ...
        NotificationCenter.default.addObserver(
            ...
            name: NSNotification.Name("APIConfigurationDidChange"),
            object: nil
        )
    }
    ...
    private func checkConfiguration() {
        isConfigured = apiConfiguration.isGoogleBooksConfigured
        
        if !isConfigured {
            configurationError = "Google Books API key is not configured. Please add your API key to Config.plist"
            print("⚠️ GoogleBooksService: API key not configured")
        } else {
            configurationError = nil
            print("✅ GoogleBooksService: API key configured successfully")
        }
    }
    ...
    private func getAPIKey() throws -> String {
        return try apiConfiguration.getGoogleBooksAPIKey()
    }
    
    /// Check if the service is available and properly configured
    public var isServiceAvailable: Bool {
        return isConfigured && (try? getAPIKey()) != nil
    }
    
    /// Get current configuration status for debugging
    public func getConfigurationStatus() -> String {
        if isConfigured {
            return "✅ Google Books API is properly configured"
        } else {
            return configurationError ?? "❌ Google Books API configuration unknown"
        }
    }
    ...
    private func performSearchWithParams(
        ...
        guard let apiKey = try? getAPIKey() else {
            let error = GoogleBooksError.configurationError("Google Books API key not configured")
            print("❌ GoogleBooksService: API key not available")
    ...
    func fetchBookById(_ id: String, completion: @escaping (Result<GoogleBook, Error>) -> Void) {
        ...
        guard let apiKey = try? getAPIKey() else {
            let error = GoogleBooksError.configurationError("Google Books API key not configured")
            print("❌ GoogleBooksService: API key not available for fetchBookById")
            completion(.failure(error))
            return
        }
...
enum GoogleBooksError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .apiError(let message):
            return "Google Books API error: \(message)"
    ...
    var recoverySuggestion: String? {
        ...
        case .configurationError:
            return "Please add your Google Books API key to Config.plist"
        default:
            return nil
        }
    }
